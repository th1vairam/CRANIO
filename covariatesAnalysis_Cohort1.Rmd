...
knit2synapse::knitToFolderEntity(file = "./covariatesAnalysis_Cohort1.Rmd",
                                 parentId = "syn2820780",
                                 folderName = "Case control and subtype differential expression (original cohort)")
```
...
Gx = Diagnosis + libraryBatch + Sex + RNA\\_days\\_to\\_harvest + PCT\\_CORRECT\\_STRAND\\_READS + 
     PCT\\_INTRONIC\\_BASES + MEDIAN\\_CV\\_COVERAGE + Age\\_mos. + PCT\\_INTERGENIC\\_BASES + 
     Initial\\_growth\\_duration\\_days + PCT\\_UTR\\_BASES + MEDIAN\\_5PRIME\\_TO\\_3PRIME\\_BIAS 
...
Also, since we are intrested in the sex variations we are explicitly modeling sex

Following model was used

Gx = Diagnosis x RNA\\_days\\_to\\_harvest x Sex + libraryBatch +  PCT\\_CORRECT\\_STRAND\\_READS + 
     PCT\\_INTRONIC\\_BASES + MEDIAN\\_CV\\_COVERAGE + Age\\_mos. + PCT\\_INTERGENIC\\_BASES + 
     Initial\\_growth\\_duration\\_days + PCT\\_UTR\\_BASES + MEDIAN\\_5PRIME\\_TO\\_3PRIME\\_BIAS 

Genes that are differentially expressed at an FDR <= 0.05 are
```{r diff.exp2, fig.height=16, fig.width=18, cache=FALSE}
# Get design matrix
COVARIATES$Diagnosis.Sex = factor(paste(COVARIATES$Diagnosis,COVARIATES$Sex, sep = '.'))
DESIGN = getDesignMatrix(COVARIATES[, c('Diagnosis.Sex', setdiff(postAdjustCovars, 'Sex')), drop = F], Intercept = F)
ind = grep('Diagnosis.Sex', colnames(DESIGN$design))
DESIGN$design[,ind] = DESIGN$design[,ind] * DESIGN$design[,'RNA_days_to_harvest']
colnames(DESIGN$design)[ind] = paste(colnames(DESIGN$design)[ind], 'RNA_days_to_harvest', sep = '.')
DESIGN$design = DESIGN$design[,setdiff(colnames(DESIGN$design), 'RNA_days_to_harvest')]
DESIGN$design = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
colnames(DESIGN$design) = c("Control.F.RDH", "Control.M.RDH", "Coronal.F.RDH", "Coronal.M.RDH", "Lambdoid.F.RDH", "Lambdoid.M.RDH", "Metopic.F.RDH", "Metopic.M.RDH", "Sagittal.F.RDH", "Sagittal.M.RDH", "batch1", "batch2", "batch3", "PCT_CORRECT_STRAND_READS", "PCT_INTRONIC_BASES", "MEDIAN_CV_COVERAGE", "Age_mos.", "Initial_growth_duration_days", "PCT_INTERGENIC_BASES", "PCT_UTR_BASES", "MEDIAN_5PRIME_TO_3PRIME_BIAS")
...
# Get residual gene expression
RESIDUAL.GENE_EXPRESSION = residuals.MArrayLM(FIT, CQN.GENE_EXPRESSION$E)
varsToAddIn = grep('.RDH', colnames(FIT$coefficients), value = T)
RESIDUAL.GENE_EXPRESSION = RESIDUAL.GENE_EXPRESSION +
  (FIT$coefficients[,varsToAddIn] %*% t(FIT$design[,varsToAddIn]))

# Fit contrast
contrast = c("(Coronal.F.RDH+Coronal.M.RDH-Control.F.RDH-Control.M.RDH)/2",
             "(Lambdoid.F.RDH+Lambdoid.M.RDH-Control.F.RDH-Control.M.RDH)/2",
             "(Metopic.F.RDH+Metopic.M.RDH-Control.F.RDH-Control.M.RDH)/2",
             "(Sagittal.F.RDH+Sagittal.M.RDH-Control.F.RDH-Control.M.RDH)/2",
             "(Coronal.F.RDH+Coronal.M.RDH+Lambdoid.F.RDH+Lambdoid.M.RDH+Metopic.F.RDH+Metopic.M.RDH+Sagittal.F.RDH+Sagittal.M.RDH)/8 - (Control.F.RDH+Control.M.RDH)/2",
             "(Coronal.F.RDH+Lambdoid.F.RDH+Metopic.F.RDH+Sagittal.F.RDH)/4 - Control.F.RDH",
             "(Coronal.M.RDH+Lambdoid.M.RDH+Metopic.M.RDH+Sagittal.M.RDH)/4 - Control.M.RDH",
             "(Coronal.F.RDH+Lambdoid.F.RDH+Metopic.F.RDH+Sagittal.F.RDH)/4-(Coronal.M.RDH+Lambdoid.M.RDH+Metopic.M.RDH+Sagittal.M.RDH)/4",
             "Control.F.RDH-Control.M.RDH",
             "Coronal.F.RDH-Control.F.RDH",
             "Coronal.M.RDH-Control.M.RDH",
             "(Coronal.F.RDH+Coronal.M.RDH-Sagittal.F.RDH-Sagittal.M.RDH)/2",
             "(Coronal.F.RDH+Coronal.M.RDH-Metopic.F.RDH-Metopic.M.RDH)/2",
             "(Coronal.F.RDH+Coronal.M.RDH-Lambdoid.F.RDH-Lambdoid.M.RDH)/2",
             "(Sagittal.F.RDH+Sagittal.M.RDH-Metopic.F.RDH-Metopic.M.RDH)/2",
             "(Sagittal.F.RDH+Sagittal.M.RDH-Lambdoid.F.RDH-Lambdoid.M.RDH)/2",
             "(Lambdoid.F.RDH+Lambdoid.M.RDH-Metopic.F.RDH-Metopic.M.RDH)/2",
             "Coronal.M.RDH-Sagittal.M.RDH",
             "Coronal.F.RDH-Sagittal.F.RDH") %>%
  makeContrasts(contrasts=.,
                levels = colnames(FIT$coefficients))
FIT.CONTR = contrasts.fit(FIT, contrasts=contrast)
FIT.CONTR = eBayes(FIT.CONTR)
...
names(DE) = c("Coronal-Control", "Lambdoid-Control", "Metopic-Control", "Sagittal-Control",
              "Cranio-Control", "Cranio.F-Control.F", "Cranio.M-Control.M", "Cranio.F-Cranio.M",
              "Control.F-Control.M", "Coronal.F-Control.F", "Coronal.M-Control.M",
              "Coronal-Sagittal", "Coronal-Metopic", "Coronal-Lambdoid",
              "Sagittal-Metopic", "Sagittal-Lambdoid", "Lambdoid-Metopic",
              "Coronal.M-Sagittal.M", "Coronal.F-Sagittal.F")

DE3 = DE %>% 
  rbindlist(idcol = 'Comparison') %>%
  dplyr::mutate(Model = 'DxxRNADaysToharvest') %>%
  tidyr::separate(Comparison, c('reference','against'), sep = '-') %>%
  left_join(GENE.PARAM %>%
              dplyr::select(ensembl_gene_id, hgnc_symbol, percentage_gc_content, gene.length) %>%
              unique()) %>%
  dplyr::mutate(Direction = logFC/abs(logFC),
                Direction = factor(Direction, c(-1,1), c('-1' = 'DOWN', '1' = 'UP')),
                Direction = as.character(Direction))
DE3$Direction[DE3$adj.P.Val > 0.05] = 'NONE'
...
p = ggplot(DE3, aes(y = -log10(adj.P.Val), x = logFC, color = Direction)) + geom_point() + xlim(c(-1,1))
p = p + scale_color_manual(values = c('green','grey','red'))
p = p + facet_grid(reference+.~against, scales = 'fixed')
p

DE = rbindlist(list(DE1, DE2, DE3), use.names = T, fill = T)
```
...
enrichmentResults = plyr::ddply(DE, .(reference, against, Model), .fun = function(x, backGroundGenes, geneSets){
...
CQN.GENE_EXPRESSION$E %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CRANIO1_winlcpm.tsv', sep = '\t', row.names=F, quote=F)
WLCOUNT_OBJ = File('CRANIO1_winlcpm.tsv', name = 'Winsorised logCPM', parentId = CODE$properties$id)
annotations(WLCOUNT_OBJ) = annotations(CODE)
annotations(WLCOUNT_OBJ)$dataSubType = 'winLcpm'
WLCOUNT_OBJ = synStore(WLCOUNT_OBJ, activity = synGetActivity(COV_OBJ$properties$id))
...
write.table(DE, file = 'CRANIO1_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
...
ENRICH_OBJ = File('CRANIO2_EnrichmentAnalysis.tsv', name = 'Enrichment Analysis (Differential Expression)', 
                  parentId = CODE$properties$id)
annotations(ENRICH_OBJ) = annotations(CODE)
annotations(ENRICH_OBJ)$dataSubType = 'enrichAnlz'
