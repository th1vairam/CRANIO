...
Gx = Diagnosis x RNA\\_days\\_to\\_harvest + libraryBatch + Sex +  PCT\\_CORRECT\\_STRAND\\_READS + 
     PCT\\_INTRONIC\\_BASES + MEDIAN\\_CV\\_COVERAGE + Age\\_mos. + PCT\\_INTERGENIC\\_BASES + 
     Initial\\_growth\\_duration\\_days + PCT\\_UTR\\_BASES + MEDIAN\\_5PRIME\\_TO\\_3PRIME\\_BIAS 
...
# Fit contrast
contrast = paste0('Diagnosis',levels(COVARIATES$Diagnosis),'.RNA_days_to_harvest') %>%
  combn(2) %>%
  apply(2, function(x){
    paste(x, collapse = '-')
  }) %>%
  c('DiagnosisControl.RNA_days_to_harvest-(DiagnosisCoronal.RNA_days_to_harvest+DiagnosisLambdoid.RNA_days_to_harvest+DiagnosisMetopic.RNA_days_to_harvest+DiagnosisSagittal.RNA_days_to_harvest)/4') %>%
  makeContrasts(contrasts=.,
                levels = colnames(FIT$coefficients))
FIT.CONTR = contrasts.fit(FIT, contrasts=contrast)
FIT.CONTR = eBayes(FIT.CONTR)
...
DE3 = DE %>% 
  rbindlist(idcol = 'Comparison') %>%
  dplyr::mutate(Comparison = gsub('Diagnosis','',Comparison),
                Comparison = gsub('.RNA_days_to_harvest','',Comparison),
                Comparison = gsub('[^[:alnum:]]','_', Comparison),
                Comparison = gsub('_Coronal_Lambdoid_Metopic_Sagittal__4', 'Cranio', Comparison),
                Model = 'DxxRNADaysToharvest') %>%
  tidyr::separate(Comparison, c('reference','against'), sep = '_') %>%
  left_join(GENE.PARAM %>%
              dplyr::select(ensembl_gene_id, hgnc_symbol, percentage_gc_content, gene.length) %>%
              unique()) %>%
...
ENRICH_OBJ = File('CRANIO2_EnrichmentAnalysis.tsv', name = 'Enrichment Analysis (Differential Expression)', 
                  parentId = CODE$properties$id)
annotations(ENRICH_OBJ) = annotations(ENRICH)
