---
title: "Covariate analysis of CRANIO counts (Cohort 1 with CQN normalisation and covariates adjusted)"
...
knit2synapse::createAndKnitToFolderEntity(file = "./covariatesAnalysis_Cohort1.Rmd",
                                          parentId = "syn2820780",
                                          folderName = "Case control and subtype differential expression (Cohort 1)")
```
...
METADATA.PICARD <- synapser::synGet(METADATA.PICARD_ID)$path %>%
  fread(data.table = F) %>%
...
METADATA <- synapser::synGet(METADATA_ID)$path %>%
...
GENE.PARAM = synapser::synGet('syn8449369')$path %>% 
  fread(data.table = FALSE)
...
METADATA <- METADATA %>%
  dplyr::filter(!is.na(Sex), !is.na(Sample_Type), !is.na(SampleID),
         !is.na(Age_mos.)) %>%
  dplyr::mutate(Sample_Type = gsub("Coronal Synostosis L", "Coronal", Sample_Type),
                Sample_Type = gsub("Coronal Synostosis R", "Coronal", Sample_Type),
                Diagnosis = factor(Sample_Type,
                                   levels = c("Control", "Coronal", "Lambdoid", "Metopic Synostosis", "Sagittal Synostosis"),
                                   labels = c("Control" = "Control", "Coronal" = "Coronal", "Lambdoid" = "Lambdoid", 
...
Gx = Diagnosis + RNA\\_days\\_to\\_harvest + Sex + libraryBatch + PCT\\_CORRECT\\_STRAND\\_READS + 
     Age\\_mos. + PCT\\_INTERGENIC\\_BASES + MEDIAN\\_CV\\_COVERAGE + 
     Initial\\_growth\\_duration\\_days + PCT\\_INTRONIC\\_BASES + Sample\\_Origin 
...
postAdjustCovars = c('RNA_days_to_harvest', 'Sex', 'libraryBatch', 'PCT_CORRECT_STRAND_READS',
                     'Age_mos.', 'PCT_INTERGENIC_BASES', 'MEDIAN_CV_COVERAGE', 'Initial_growth_duration_days', 
                     'PCT_INTRONIC_BASES', 'Sample_Origin')   
...
Gx = Diagnosis x RNA\\_days\\_to\\_harvest + Sex + libraryBatch + PCT\\_CORRECT\\_STRAND\\_READS + 
     Age\\_mos. + PCT\\_INTERGENIC\\_BASES + MEDIAN\\_CV\\_COVERAGE + 
     Initial\\_growth\\_duration\\_days + PCT\\_INTRONIC\\_BASES + Sample\\_Origin 
...
# Code
CODE <- synapser::Folder(name = "Case control and subtype differential expression (Cohort 1)", parentId = parentId)
...
COV_OBJ = synapser::File('CRANIO1_Covariates.tsv', name = 'Covariates', parentId = CODE$properties$id)
...
COUNT_OBJ = synapser::File('CRANIO1_Counts.tsv', name = 'Counts (filtered raw)', parentId = CODE$properties$id)
COUNT_OBJ = synasper::synStore(COUNT_OBJ, activity = synapser::synGetProvenance(COV_OBJ$properties$id))
...
LCOUNT_OBJ = synapser::File('CRANIO1_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId = CODE$properties$id)
...
tmp = synapser::synSetAnnotations(LCOUN_OBJ$properties$id, list = all.annotations)

# Store cqn offsets
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CRANIO1_offset.tsv', sep = '\t', row.names=F, quote=F)
OFFSET_OBJ = synapser::File('CRANIO1_offset.tsv', name = 'Gene length and GC content offset', parentId = CODE$properties$id)
OFFSET_OBJ = synapser::synStore(OFFSET_OBJ, activity = synGetActivity(COV_OBJ$properties$id))
all.annotations$dataSubType = 'offset'
tmp = synasper::synSetAnnotations(OFFSET_OBJ$properties$id, list = all.annotations)

# Store winsorised data
CQN.GENE_EXPRESSION$E %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CRANIO1_winlcpm.tsv', sep = '\t', row.names=F, quote=F)
WLCOUNT_OBJ = synapser::File('CRANIO1_winlcpm.tsv', name = 'Winsorised logCPM', parentId = CODE$properties$id)
WLCOUNT_OBJ = synapser::synStore(WLCOUNT_OBJ, activity = synGetActivity(COV_OBJ$properties$id))
all.annotations$dataSubType = 'winLcpm'
tmp = synasper::synSetAnnotations(WLCOUNT_OBJ$properties$id, list = all.annotations)

# Store design matrix
DESIGN$design %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = 'CRANIO1_Design.tsv', sep = '\t', row.names=F, quote=F)
DM_OBJ = synapser::File('CRANIO1_Design.tsv', name = 'Design Matrix', parentId = CODE$properties$id)
...
tmp = synasper::synSetAnnotations(DM_OBJ$properties$id, list = all.annotations)
...
nEXP_OBJ = synapser::File('CRANIO1_netResidualExpression.tsv', 
...
tmp = synasper::synSetAnnotations(nEXP_OBJ$properties$id, list = all.annotations)

# Store differential expression results
write.table(DE, file = 'CRANIO1_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = synapser::File('CRANIO1_DiffExpression.tsv', 
...
tmp = synasper::synSetAnnotations(DEXP_OBJ$properties$id, list = all.annotations)
