---
title: "Covariate analysis of CRANIO counts (validation cohort with CQN normalisation and covariates adjusted)"
...
knit2synapse::knitToFolderEntity(file = "./covariatesAnalysis_validCohort.Rmd",
                                 parentId = "syn2820780",
                                 folderName = "Case control and subtype differential expression (validation cohort)")
```
...
thisFileName <- 'covariatesAnalysis_validCohort.Rmd'
...
METADATA.MATCH <- synTableQuery(paste('select * from', METADATA.MATCH_ID))@values %>%
...
METADATA.PICARD <- read.table(synGet(METADATA.PICARD_ID)@filePath,sep = '\t', header = T) %>%
...
METADATA <- synTableQuery(paste('select * from', METADATA_ID))@values %>%
  dplyr::rename(SampleID = Sample_ID) %>%
  dplyr::left_join(METADATA.MATCH) %>%
...
METADATA <- METADATA %>%
  filter(Sample_Name %in% colnames(COUNT)) %>%
  filter(!is.na(Sex), !is.na(Diagnosis), !is.na(Sample_Name),
         Age_at_Collection != 'xxx',
         Days_originally_grown != 'xxx') %>%
  dplyr::mutate(Diagnosis = factor(Diagnosis,
                                   levels = c("Control", "Coronal", "Coronal ", "Coronal Synostosis-L", "Coronal Synostosis-R", 
                                              "Lambdoid", "Lambdoid-L", "Lambdoid-R", "Metopic", "Sagittal"),
                                   labels = c("Control" = "Control", "Coronal" = "Coronal", "Coronal " = "Coronal", 
                                              "Coronal Synostosis-L" = "Coronal", "Coronal Synostosis-R" = "Coronal", 
                                              "Lambdoid" = "Lambdoid", "Lambdoid-L" = "Lambdoid", "Lambdoid-R" = "Lambdoid", 
                                              "Metopic" = "Metopic", "Sagittal" = "Sagittal"))) %>%
  droplevels() %>%
# filter(!(Sample_Name %in% c('163541', '163707'))) %>%
  as.data.frame()
```
...
Gx = Diagnosis + RNA\_days\_to\_Harvest + Batch + Sex + PCT\\_CORRECT\\_STRAND\\_READS + Age\\_at\\_Collection +
     PCT\\_UTR\\_BASES + Days\\_originally\\_grown + MEDIAN\\_CV\\_COVERAGE + MEDIAN\\_5PRIME\\_TO\\_3PRIME\\_BIAS +
     PCT\\_INTERGENIC\\_BASES + PCT\\_INTRONIC\\_BASES 

Genes that are differentially expressed at an FDR <= 0.05 are
```{r diff.exp, fig.height=16, fig.width=18, cache=FALSE}
# Get design matrix
varsInModel = c('Batch', 'Sex', 'RNA_days_to_Harvest', 'PCT_CORRECT_STRAND_READS', 'PCT_INTRONIC_BASES', 
                'MEDIAN_CV_COVERAGE', 'Age_at_Collection', 'Days_originally_grown', 'PCT_INTERGENIC_BASES', 
                'PCT_UTR_BASES', 'MEDIAN_5PRIME_TO_3PRIME_BIAS')
DESIGN = getDesignMatrix(COVARIATES[, c(primaryVariable, varsInModel), drop = F], Intercept = F)
...
Since RNA\_days\_to\_Harvest is also strongly associated (biologically) with the case control differences, we are explicitly modeling hte case-control variations as an interaction effect

Also, since we are intrested in the sex variations we are explicitly modeling sex

Following model was used

Gx = Diagnosis x RNA\_days\_to\_Harvest x Sex + Batch + PCT\\_CORRECT\\_STRAND\\_READS + Age\\_at\\_Collection +
     PCT\\_UTR\\_BASES + Days\\_originally\\_grown + MEDIAN\\_CV\\_COVERAGE + MEDIAN\\_5PRIME\\_TO\\_3PRIME\\_BIAS +
     PCT\\_INTERGENIC\\_BASES + PCT\\_INTRONIC\\_BASES 
...
DESIGN$design = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
colnames(DESIGN$design)[1:10] = c("Control.F.RDH", "Control.M.RDH", "Coronal.F.RDH", "Coronal.M.RDH",
                                  "Lambdoid.F.RDH", "Lambdoid.M.RDH", "Metopic.F.RDH", "Metopic.M.RDH",
                                  "Sagittal.F.RDH", "Sagittal.M.RDH")
...
pl[[1]] = pl[[1]] + facet_grid(reference~.+against) + scale_color_manual(values = c('grey'))
...
pl[[2]] = pl[[2]] + facet_grid(reference~.+against) + scale_color_manual(values = c('grey'))
...
pl[[3]] = pl[[3]] + facet_grid(reference~.+against) + scale_color_manual(values = c('grey'))
...
# Code
CODE <- Folder(name = "Case control and subtype differential expression (validation cohort)", parentId = parentId)
...
COV_OBJ = File('CRANIO2_Covariates.tsv', name = 'Covariates', parentId = CODE$properties$id)
...
COUNT_OBJ = File('CRANIO2_Counts.tsv', name = 'Counts (filtered raw)', parentId = CODE$properties$id)
...
LCOUNT_OBJ = File('CRANIO2_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId = CODE$properties$id)
...
OFFSET_OBJ = File('CRANIO2_offset.tsv', name = 'Gene length and GC content offset', parentId = CODE$properties$id)
...
DM_OBJ = File('CRANIO2_Design.tsv', name = 'Design Matrix', parentId = CODE$properties$id)
...
nEXP_OBJ = File('CRANIO2_netResidualExpression.tsv', 
...
DEXP_OBJ = File('CRANIO2_DiffExpression.tsv', name = 'Differential Expression Results', 
                parentId = CODE$properties$id)
