...
METADATA <- METADATA %>%
  ...
  filter(!is.na(Sex), !is.na(Diagnosis)) %>%
...
```
`r dim(COUNT)[2]` samples from the validation cohorts are used for tha analysis. Following sample are removed `r paste(removedIDs, collapse = ',')`

### Covariate clustering
Determine relationship between covariates
```{r covariates.clustering, cache=TRUE}
primaryVariable <- "Diagnosis"
FactorCovariates <- c("Plate", "Sex", "Batch", "Diagnosis")
ContCovariates <- c("RNA_Days_to_Harvest", "PF_BASES", "PF_ALIGNED_BASES", "PCT_CODING_BASES", "PCT_UTR_BASES",
...
DE$Direction[DE$adj.P.Val > 0.05] = 'NONE'
...
### Differential expression analysis (control vs cranio)
Differential expression is performed on the primary variable by controlling for covariates identified above

Genes that are differentially expressed at an FDR <= 0.05 are
```{r diff.exp1, fig.height=6, fig.width=8, cache=FALSE}
# Get design matrix
COVARIATES$Diagnosis = as.character(COVARIATES$Diagnosis)
COVARIATES$Diagnosis[COVARIATES$Diagnosis != 'Control'] = 'Cranio'
COVARIATES$Diagnosis = factor(COVARIATES$Diagnosis)
DESIGN = getDesignMatrix(COVARIATES[, c(primaryVariable, postAdjustCovars), drop = F], Intercept = F)
DESIGN$design = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
...
RESIDUAL.GENE_EXPRESSION = residuals.MArrayLM(FIT, CQN.GENE_EXPRESSION$y+CQN.GENE_EXPRESSION$offset)
RESIDUAL.GENE_EXPRESSION = RESIDUAL.GENE_EXPRESSION + FIT$coefficients[,c("DiagnosisControl", "DiagnosisCranio")] %*% t(DESIGN$design[,c("DiagnosisControl", "DiagnosisCranio")])
...
# Get differnetial expression
DE1 = lapply(1, function(i, FIT){
  topTable(FIT, coef=i, number = 50000, confint = T) %>%
    rownameToFirstColumn('ensembl_gene_id')
}, FIT.CONTR)
names(DE1) = colnames(contrast)
...
pl = list()
pl[[1]] = ggplot(DE1, aes(x = log10(gene.length), y = logFC, color = Direction)) + geom_point() 
...
pl[[2]] = ggplot(DE1, aes(x = percentage_gc_content, y = logFC, color = Direction)) + geom_point()
...
pl[[3]] = ggplot(DE1, aes(x = AveExpr, y = logFC, color = Direction)) + geom_point()
